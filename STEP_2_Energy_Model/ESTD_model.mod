...
set RE_EQUIV_FUELS within RESOURCES; # imported RE_EQUIV_FUELS.
...
set LAYERS := (RESOURCES diff RE_EQUIV_FUELS diff EXPORT) union END_USES_TYPES; # Layers are used to balance resources/products in the system
...
param c_grid_extra >=0; # Cost to reinforce the grid due to IRE penetration [M€/GW of (PV + Wind)].
...
## Min CO2 achievable constaints
#-------------------------------
# subject to CO2_MIN_ACHIEVABLE_WOOD:
# 	sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} F_t["WOOD",h,td] = avail["WOOD"];
# subject to CO2_MIN_ACHIEVABLE_WET_BIOMASS:
# 	sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} F_t["WET_BIOMASS",h,td] = avail["WET_BIOMASS"];
# subject to CO2_MIN_ACHIEVABLE_WASTE:
# 	sum{t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} F_t["WASTE",h,td] = avail["WASTE"];
...
subject to end_uses_t {l in LAYERS, h in HOURS, td in TYPICAL_DAYS}:
	End_uses [l, h, td] = (if l == "ELECTRICITY" 
		...
		else (if l == "NON_ENERGY_HVC" then
			end_uses_input[l] / total_time
		else (if l == "NON_ENERGY_NH3" then
			end_uses_input[l] / total_time
		else (if l == "NON_ENERGY_CH3OH" then
			end_uses_input[l] / total_time
		else 
...
# [Eq. 21]  Extra grid cost for integrating 1 GW of RE is estimated to 367.8M€ per GW of intermittent renewable (27b€ to integrate the overall potential) 
...
/* Not integrated at this stage :-/
# [Eq. XX] Mob infrastructures
subject to mob_infrastructures_motorways  :
	F ["MOTORWAYS"] =  0.4  * sum {j in TECHNOLOGIES_OF_END_USES_TYPE["MOB_PRIVATE"], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} 
								  (F_t[j,h,td])  / 126399 
						   + 0.1  * sum {j in TECHNOLOGIES_OF_END_USES_TYPE["MOB_PUBLIC"] diff {"TRAIN_PUB","TRAMWAY_METRO"}, t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} 
								  (F_t[j,h,td]) / 18012
						   + 0.5  * sum {j in TECHNOLOGIES_OF_END_USES_TYPE["MOB_FREIGHT_ROAD"], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} 
								  (F_t[j,h,td]) / 47980; # 126399 is the total Mkm-pass supplied by private cars. 18012 is the same supplied by MOB PUBLIC - train - tram.47980 same for freight ROAD

# [Eq. XX] Mob infrastructures
subject to mob_infrastructures_roads  :
	F ["ROADS"] =      0.6 * sum {j in TECHNOLOGIES_OF_END_USES_TYPE["MOB_PRIVATE"], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} 
								  (F_t[j,h,td]) / 126399 
						   + 0.2 * sum {j in TECHNOLOGIES_OF_END_USES_TYPE["MOB_PUBLIC"] diff {"TRAIN_PUB","TRAMWAY_METRO"}, t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} 
								  (F_t[j,h,td]) / 18012
						   + 0.2 * sum {j in TECHNOLOGIES_OF_END_USES_TYPE["MOB_FREIGHT_ROAD"], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} 
								  (F_t[j,h,td]) / 47980; # 126399 is the total Mkm-pass supplied by private cars. 18012 is the same supplied by MOB PUBLIC - train - tram.47980 same for freight ROAD

# [Eq. XX] Mob infrastructures
subject to mob_infrastructures_rails  :
	F ["RAILWAYS"] =      0.6 * sum {j in {"TRAIN_PUB","TRAMWAY_METRO"}, t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} 
								  (F_t[j,h,td]) / 13588
						   + 0.4 * sum {j in TECHNOLOGIES_OF_END_USES_TYPE["MOB_FREIGHT_RAIL"], t in PERIODS, h in HOUR_OF_PERIOD[t], td in TYPICAL_DAY_OF_PERIOD[t]} 
								  (F_t[j,h,td]) / 8250; # 126399 is the total Mkm-pass supplied by private cars. 18012 is the same supplied by MOB PUBLIC - train - tram.47980 same for freight ROAD
*/
...
subject to Freight_shares :
	Share_freight_train + Share_freight_road + Share_freight_boat = 1; # Should not be required... But don't know it is required.
...
# [Eq. 32] Peak in decentralized heating
...
# [Eq. 33] Calculation of max heat demand in DHN (1st constraint required to linearised the max function)
...
# [Eq. 34]  constraint to reduce the GWP subject to Minimum_gwp_reduction :
...
# [Eq. 37] Energy efficiency is a fixed cost
